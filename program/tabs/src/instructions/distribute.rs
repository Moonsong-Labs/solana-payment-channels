...
pub fn process(
    ...
    if !matches!(status, ChannelStatus::Open | ChannelStatus::Sealed) {
        return Err(TabsError::ChannelNotDistributable.into());
    }

    // Identity.
    if accs.payer.address() != &ch.payer {
        return Err(TabsError::InvalidChannelPayer.into());
    }
    if accs.mint.address() != &ch.mint {
        return Err(TabsError::InvalidChannelMint.into());
    }
    ...
    if accs.rent_payer.address() != &ch.rent_payer {
        return Err(TabsError::InvalidChannelRentPayer.into());
    }
    ...
    if digest != ch.distribution_hash {
        return Err(TabsError::InvalidDistributionHash.into());
    }
    ...
    if accs.recipient_token_accounts.len() != args.recipients.entries.len() {
        return Err(TabsError::RecipientAccountCountMismatch.into());
    }
    ...
    if status == ChannelStatus::Open && settlement.is_fully_accounted()? {
        return Err(TabsError::NothingToDistribute.into());
    }
    ...
    if is_sealed {
        ...
        let treasury_sweep = escrow_at_entry
            ...
            .ok_or(TabsError::DistributeBalanceCalculationOverflow)?;
    ...
    fn settlement_watermarks_reject_payout_watermark_above_settled() {
        ...
        assert_eq!(
            ...
            Err(ProgramError::from(
                TabsError::DistributePoolOverflow
            ))
        );
        assert_eq!(
