        ...
        match TokenProgramKind::from_address(self.address())? {
            TokenProgramKind::Spl => {
                Ok(pinocchio_token::state::Account::from_account_view(account)
                    .map_err(|_| TabsError::MalformedMintTokenAccountData)?
                    .amount())
            }
...
impl<'a> TreasuryTokenAccountView<'a, Unchecked> {
    ...
    ) -> Result<TreasuryTokenAccountView<'a, Checked>, TabsError> {
        self.inner
            ...
            .map_err(|err| match err {
                ...
                | AccountValidationError::AccountNotInitialized => {
                    TabsError::TreasuryAccountMismatch
                }
                AccountValidationError::MalformedTokenAccountData => {
                    TabsError::InvalidTreasuryTokenAccount
                }
                AccountValidationError::TokenExtensionError(_) => {
                    TabsError::InvalidTreasuryTokenExtensions
                }
            })?;
...
impl TokenProgramKind {
    pub fn from_address(address: &Address) -> Result<Self, TabsError> {
        if address == &pinocchio_token::ID {
            Ok(Self::Spl)
        } else if address == &pinocchio_token_2022::ID {
            Ok(Self::Token2022)
        } else {
            Err(TabsError::InvalidMintTokenProgram)
        }
    }
...
impl PayoutBeneficiary {
    ...
    /// regardless.
    fn map_account_error(self, err: AccountValidationError) -> TabsError {
        match (self, err) {
            (
                Self::Recipient,
                AccountValidationError::AddressMismatch | AccountValidationError::OwnerMismatch,
            ) => TabsError::RecipientAccountMismatch,
            (
                Self::Recipient,
                AccountValidationError::MalformedTokenAccountData
                | AccountValidationError::AccountNotInitialized,
            ) => TabsError::InvalidRecipientTokenAccount,
            (Self::Recipient, AccountValidationError::TokenExtensionError(_)) => {
                TabsError::InvalidRecipientTokenExtensions
            }
            (
                Self::Payee,
                AccountValidationError::AddressMismatch | AccountValidationError::OwnerMismatch,
            ) => TabsError::PayeeAccountMismatch,
            (
                Self::Payee,
                AccountValidationError::MalformedTokenAccountData
                | AccountValidationError::AccountNotInitialized,
            ) => TabsError::InvalidPayeeTokenAccount,
            (Self::Payee, AccountValidationError::TokenExtensionError(_)) => {
                TabsError::InvalidPayeeTokenExtensions
            }
            (
                Self::Payer,
                AccountValidationError::AddressMismatch | AccountValidationError::OwnerMismatch,
            ) => TabsError::PayerAccountMismatch,
            (
                Self::Payer,
                AccountValidationError::MalformedTokenAccountData
                | AccountValidationError::AccountNotInitialized,
            ) => TabsError::InvalidPayerTokenAccount,
            (Self::Payer, AccountValidationError::TokenExtensionError(_)) => {
                TabsError::InvalidPayerTokenExtensions
            }
        }
    }
}
        ...
        let decimals = match kind {
            TokenProgramKind::Spl => {
                ...
                pinocchio_token::state::Mint::from_account_view(&mint)
                    .map_err(|_| TabsError::MintAccountMismatch)?
                    .decimals()
            }
        ...
        if kind == TokenProgramKind::Token2022 {
            ...
            if data.len() > base_layout::MINT_LEN {
                ...
                let all_zero = data[base_layout::MINT_LEN..tlv::ACCOUNT_TYPE_OFFSET]
                    ...
                    .all(|b| *b == 0);
                if !all_zero {
                    return Err(TabsError::MalformedMintTokenAccountData);
                }
                scan_tlv_extensions::<MintExtensionPolicy>(&data[tlv::START..]).map_err(|err| {
                    match err {
                        ...
                        | TokenExtensionError::UnsupportedTokenExtension => {
                            TabsError::MalformedMintTokenExtensions
                        }
                    }
                })?;
            }
...
impl<'a> ChannelContext<'a> {
    pub fn new(
        ...
        channel_token_account
            ...
            .map_err(|err| match err {
                ...
                | AccountValidationError::OwnerMismatch
                | AccountValidationError::AccountNotInitialized => {
                    TabsError::ChannelAccountMismatch
                }
                AccountValidationError::MalformedTokenAccountData => {
                    TabsError::InvalidChannelTokenAccount
                }
                AccountValidationError::TokenExtensionError(_) => {
                    TabsError::InvalidChannelTokenExtensions
                }
            })?;
    ...
    pub fn new_uninit(
        ...
        channel_token_account
            ...
            .map_err(|_| TabsError::ChannelAccountMismatch)?;
