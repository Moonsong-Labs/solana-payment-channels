...
impl<'a> Transfer<'a> {
    ...
    fn reserve_transfer_amount(&mut self, amount: u64) -> Result<(), TabsError> {
        if self.pending_len >= MAX_PENDING {
            return Err(TabsError::DistributeTransferQueueOverflow);
        }
    ...
    pub fn push(&mut self, to: &'a AccountView, amount: u64) -> Result<(), TabsError> {
        if amount == 0 {
            return Ok(());
        }
