...
pub fn verify_voucher(
    ...
    let prev_idx = current
        .checked_sub(1)
        .ok_or(TabsError::MissingEd25519Verification)?;
    let ix = sysvar
        .load_instruction_at(prev_idx as usize)
        .map_err(|_| TabsError::MissingEd25519Verification)?;
    if ix.get_program_id() != &crate::ed25519::PROGRAM_ID {
        return Err(TabsError::MissingEd25519Verification.into());
    }
    let parsed = ed25519_ix::parse(ix.get_instruction_data())
        .map_err(|_| TabsError::MalformedEd25519Instruction)?;
...
fn verify_parsed(
    ...
    if voucher.magic != VOUCHER_MAGIC {
        return Err(TabsError::VoucherBadMagic.into());
    }
...
mod tests {
    ...
    fn expect_err(result: Result<u64, ProgramError>, expected: TabsError) {
        match result {
    ...
    fn cumulative_equals_settled() {
        ...
        expect_err(
            verify_parsed(&CHANNEL_ID, &ch, &parsed, 0),
            TabsError::VoucherWatermarkNotMonotonic,
        );
    }
    ...
    fn precompile_pubkey_not_authorized_signer() {
        ...
        expect_err(
            verify_parsed(&CHANNEL_ID, &ch, &parsed, 0),
            TabsError::VoucherSignerMismatch,
        );
    }
}
