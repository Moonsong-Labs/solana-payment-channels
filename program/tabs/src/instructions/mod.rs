...
mod tests {
    ...
    fn discriminators_match_variants() {
        ...
        assert_eq!(
            tag(&TabsInstruction::Open(open)),
            open::DISCRIMINATOR,
        );
        assert_eq!(
            tag(&TabsInstruction::Settle),
            settle::DISCRIMINATOR,
        );
        assert_eq!(
            tag(&TabsInstruction::TopUp(&top_up)),
            top_up::DISCRIMINATOR,
        );
        assert_eq!(
            tag(&TabsInstruction::SettleAndSeal(&saf)),
            settle_and_seal::DISCRIMINATOR,
        );
        assert_eq!(
            tag(&TabsInstruction::RequestClose),
            request_close::DISCRIMINATOR,
        );
        assert_eq!(tag(&TabsInstruction::Seal), seal::DISCRIMINATOR,);
        assert_eq!(
            tag(&TabsInstruction::Distribute(distribute)),
            distribute::DISCRIMINATOR,
        );
        assert_eq!(
            tag(&TabsInstruction::WithdrawPayer),
            withdraw_payer::DISCRIMINATOR,
        );
        assert_eq!(
            tag(&TabsInstruction::Reclaim),
            reclaim::DISCRIMINATOR,
        );
        assert_eq!(
            tag(&TabsInstruction::EmitEvent),
