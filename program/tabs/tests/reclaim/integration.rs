...
fn custom_err(e: TabsError) -> ProgramResult {
...
fn open_status_rejects() {
    assert_eq!(
        ...
        custom_err(TabsError::InvalidChannelStatus),
    );
}
...
fn wrong_rent_payer_rejects() {
    ...
    assert_eq!(
        ...
        .run(),
        custom_err(TabsError::InvalidChannelRentPayer),
    );
}
...
fn gate_rejects_at_genesis_slot() {
    ...
    assert_eq!(
        ...
        .run(),
        custom_err(TabsError::ChannelCloseTooEarly),
    );
}
