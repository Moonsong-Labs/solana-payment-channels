...
use tabs::TabsError;
use tabs::constants::OPEN_SLOT_WINDOW;
use tabs::instructions::distribute::DISCRIMINATOR;
use tabs::instructions::open::DISCRIMINATOR as OPEN_DISCRIMINATOR;
use tabs_client::instructions::{Settle, WithdrawPayer};
use tabs_client::types::{
...
fn bad_preimage_hash() {
    ...
    expect_custom_err(res, TabsError::InvalidDistributionHash);
}
...
fn unsupported_token_2022_mint_extensions_reject_without_state_changes() {
    for (extension_type, value_len) in [
        ...
        expect_custom_err(res, TabsError::MalformedMintTokenExtensions);
        assert_eq!(
            read_payout_watermark(&s.svm, &s.channel),
...
fn malformed_token_2022_account_extension_rejects_without_state_changes() {
    ...
    expect_custom_err(res, TabsError::InvalidRecipientTokenExtensions);
    assert_eq!(token_balance(&s.svm, &s.recipient_atas[0]), 0);
    assert_eq!(token_balance(&s.svm, &s.payee_ata), 0);
    assert_eq!(token_balance(&s.svm, &s.treasury_ata), 0);
    assert_eq!(
...
fn wrong_recipient_ata() {
    ...
    expect_custom_err(s.send(ix), TabsError::RecipientAccountMismatch);
}
...
fn wrong_treasury_ata() {
    ...
    expect_custom_err(s.send(ix), TabsError::TreasuryAccountMismatch);
}
...
fn wrong_token_program() {
    ...
    expect_custom_err(s.send(ix), TabsError::InvalidMintTokenProgram);
}
...
fn status_closing_rejects() {
    ...
    expect_custom_err(
        s.send(s.distribute_ix()),
        TabsError::ChannelNotDistributable,
    );
}
...
fn num_recipients_exceeds_max() {
    ...
    expect_custom_err(s.send(ix), TabsError::InvalidRecipientCount);
}
...
fn recipient_tail_length_mismatch_rejects() {
    ...
    expect_custom_err(
        s.send(ix),
        TabsError::RecipientAccountCountMismatch,
    );
}
...
fn bps_sum_equals_10000_still_validates_payee_ata() {
    ...
    expect_custom_err(s.send(ix), TabsError::PayeeAccountMismatch);
}
...
fn settle_pair_for(
    s: &Scenario,
    voucher: &tabs_client::types::VoucherArgs,
    target: &Pubkey,
) -> [Instruction; 2] {
    let payload = voucher_payload(voucher);
...
fn reopen_same_tuple_after_close_lands_at_new_address_and_old_voucher_dies() {
    ...
    expect_custom_err(
        s.svm.send_transaction(tx),
        TabsError::VoucherChannelMismatch,
    );
    read_channel(&s.svm, &new_channel, |ch| {
        assert_eq!(
            ch.settled(),
...
fn sealed_distribute_inside_window_takes_two_phase_path() {
    ...
    expect_custom_err(
        ...
        TabsError::ChannelCloseTooEarly,
    );
