...
use tabs_client::instructions::{TopUp, TopUpInstructionArgs};
use tabs_client::types::TopUpArgs;
...
fn top_up_zero_amount_rejects() {
    ...
    expect_custom_err(
        ...
        TabsError::DepositMustBeNonZero,
    );
}
...
fn top_up_unsigned_payer_rejects() {
    ...
    expect_custom_err(
        svm.send_transaction(tx),
        TabsError::MissingRequiredSignature,
    );
}
