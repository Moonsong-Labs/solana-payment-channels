...
use tabs::TabsError;
use tabs::instructions::open::MAX_DISTRIBUTION_RECIPIENTS;
...
fn zero_grace_period_rejected() {
    assert_eq!(
        ...
        ProgramResult::Failure(ProgramError::Custom(
            TabsError::GracePeriodMustBeNonZero as u32
        )),
    );
}
...
fn zero_recipients_passes_arg_validation() {
    ...
    assert_eq!(
        ...
        ProgramResult::Failure(ProgramError::Custom(
            TabsError::ChannelAddressMismatch as u32
        )),
    );
}
...
fn too_many_recipients_rejected() {
    assert_eq!(
        ...
        ProgramResult::Failure(ProgramError::Custom(
            TabsError::InvalidRecipientCount as u32
        )),
    );
}
...
fn payer_equals_payee_rejected() {
    ...
    assert_eq!(
        ...
        .run(),
        ProgramResult::Failure(ProgramError::Custom(
            TabsError::PayerPayeeMustDiffer as u32
        )),
    );
}
...
fn off_curve_authorized_signer_rejected_before_mutation() {
    ...
    expect_custom_err(
        ...
        TabsError::InvalidAuthorizedSigner,
    );
...
fn bps_zero_rejected() {
    assert_eq!(
        ...
        .run(),
        ProgramResult::Failure(ProgramError::Custom(
            TabsError::InvalidSplitConfig as u32
        )),
    );
}
...
fn duplicate_recipient_rejected() {
    ...
    assert_eq!(
        ...
        .run(),
        ProgramResult::Failure(ProgramError::Custom(
            TabsError::DuplicateRecipient as u32
        )),
    );
}
