...
use tabs::state::Channel;
use tabs::state::channel::ChannelStatus;
use tabs::state::{AccountDiscriminator, CURRENT_CHANNEL_VERSION};
use tabs_client::instructions::Reclaim;
...
/// Tabs program ID.
pub const PROGRAM_ID: Pubkey = Pubkey::new_from_array(*tabs::ID.as_array());
...
pub fn ed25519_program_id() -> Pubkey {
    Pubkey::new_from_array(*tabs::ed25519::PROGRAM_ID.as_array())
}

pub fn event_authority() -> Pubkey {
    Pubkey::find_program_address(
        &[tabs::event_engine::EVENT_AUTHORITY_SEED],
        &PROGRAM_ID,
    )
    .0
}
...
pub fn treasury_owner() -> Pubkey {
    Pubkey::new_from_array(*tabs::constants::TREASURY_OWNER.as_array())
}
...
/// Opens a tab with a single 100% distribution recipient and
...
fn program_binary_path() -> String {
    std::env::var("TABS_SO")
        .unwrap_or_else(|_| "../../target/deploy/tabs.so".into())
}
