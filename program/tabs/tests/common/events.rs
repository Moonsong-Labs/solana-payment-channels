...
use tabs::event_engine::{
    EVENT_DISCRIMINATOR_LEN, EVENT_IX_TAG_LE, EventDiscriminator,
};
use tabs_client::types::{Opened, PayoutRedirected};
...
impl TestEvent for Opened {
    const DISCRIMINATOR: [u8; 8] =
        <tabs::events::Opened as EventDiscriminator>::DISCRIMINATOR;
}

impl TestEvent for PayoutRedirected {
    const DISCRIMINATOR: [u8; 8] =
        <tabs::events::PayoutRedirected as EventDiscriminator>::DISCRIMINATOR;
}
