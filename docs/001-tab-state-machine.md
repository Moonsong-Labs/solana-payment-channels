...
**PDA payees.** `open` intentionally does not require `payee` to be on-curve. Direct `settleAndSeal` transactions still require a signer equal to `Channel.payee`, so program-derived address (PDA) payees can use the cooperative-close path only when their owning program invokes tabs via CPI with signer seeds. Permissionless `settle`, `seal`, and `distribute` do not depend on a payee signature.
