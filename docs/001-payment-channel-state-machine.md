...
This is intentional. Hard-rejecting any mint with a live freeze authority would exclude the majority of real-world stablecoins (USDC, USDT, EURC, …), all of which retain a freeze authority controlled by the issuer. This does not make every stablecoin compatible: Token-2022 mints must also satisfy the extension allow-list above, so PYUSD and USDG are currently rejected. The trust decision is therefore pushed off-chain: a merchant accepting payments in mint `M` is implicitly accepting that `M`'s freeze authority can wedge any channel denominated in `M`, and should only do so for mints whose freeze authority they consider acceptably governed.
